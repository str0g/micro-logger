...
size_t
to_kilobytes (size_t bytes)
{
  return bytes / (1024);
}

size_t
to_megabytes (size_t bytes)
{
  return bytes / (1024 * 1024);
}

double
to_bandwidth_mb_s (size_t bytes, long time)
{
...
template <typename T>
auto
bench (T obj, std::string_view func, std::string_view desc,
...
{
  {
    ...
    auto exec_time = bench (
        [&] () {
          for (auto &data : data_set)
            {
              volatile auto ignore
                  = micro_logger::bytes_to_integral<int> (data);
            }
        },
    ...
    auto exec_time = bench (
        [&] () {
          for (auto &data : data_set)
            {
              volatile auto ignore
                  = micro_logger::bytes_to_integral<long> (data);
            }
        },
